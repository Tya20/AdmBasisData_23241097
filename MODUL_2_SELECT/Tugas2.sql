...
-- membuat 10 query SELECT dar database yang dibuat 
-- 1 mengambil kolom id_buku pada tabel buku
SELECT id_buku from buku;

-- 2 mengambil lebih 1 dari kolom (judul_buku dan tahun_terbit) pada tabel buku
SELECT judul_buku, tahun_terbit FROM buku;

-- 3 mengambil seluruh kolom pada tabel kategori
SELECT * FROM kategori;

-- 4 menggunakan PREFIX nama tabel untuk kolom email pada tabel pelanggan
SELECT pelanggan.email FROM pelanggan;

-- 5 menggunakan nama database dan tabel sebagai PRFIX untuk kolom alamat pada tabel penerbit
SELECT tokobuku.penerbit.alamat FROM penerbit;

-- 6 menggunakan nama database dan tabel PREFIX  untuk kolom id_supplier,nama_supplier
SELECT tokobuku.supplier.id_supplier, nama_supplier FROM supplier;

-- 7 mengganti sementara nama kolom ID_transaksi dengan ALIAS pada tabel transaksi
SELECT ID_transaksi AS DT FROM transaksi;

-- 8 mengganti sementara nama kolom id_pelanggan dan nama dengan ALIAS pada tabel transaksi
SELECT id_pelanggan AS DP, nama AS NM FROM transaksi;

-- 9 mengkombinasikan PREFIX dan ALIAS pada kolom id_buku pada tabel buku
SELECT buku.id_buku AS BK FROM buku;

-- 10 mengkombinasikan PREFIX dan ALIAS dari database pada tabel kategori untuk kolom id_kt, namakt
SELECT tokobuku.kategori.id_kt AS DK, namakt AS NK FROM kategori;
